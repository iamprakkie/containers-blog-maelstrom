...
then
    ...
    log 'R' "Slack incoming webhook URL is invalid. Check the input value and provide a valid full webhook URL along with protocol https://.${NC}"
    exit 1
fi
...
then
    ...
    log 'R' "Slack channel name is invalid. Slack channel name should not contain any spaces.${NC}"
    exit 1
fi
